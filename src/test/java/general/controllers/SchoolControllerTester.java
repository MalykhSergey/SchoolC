...
public class SchoolControllerTester {
    ...
    public void testAddSchoolPostOnCompleted() {
        Mockito.when(schoolService.checkSchoolName(any())).thenReturn(new CheckDataBoolAnswer(true,""));
    ...
    public void testAddSchoolPostOnSchoolError(){
        Mockito.when(schoolService.checkSchoolName(any())).thenReturn(new CheckDataBoolAnswer(false,"Error!"));
        schoolController.addSchoolPost(schoolForm,model);
